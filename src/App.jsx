...
const Tooltip = ({ text, children, hideIcon }) => (
  <div className="group relative inline-flex items-center gap-1.5 cursor-help">
    {children}
    {!hideIcon && <Info className="w-3.5 h-3.5 text-slate-400 hover:text-black transition-colors" />}
...
export default function App() {
  ...
  const totalWeight = useMemo(() => {
    return Object.values(weights).reduce((sum, weight) => sum + (Number(weight) || 0), 0);
  }, [weights]);

  return (
    <div className="min-h-screen bg-neutral-100 text-neutral-900 font-sans p-4 md:p-8 selection:bg-neutral-300">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between pb-2 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-neutral-500 tracking-widest uppercase mt-1">
                Professional Portfolio Risk Dashboard
              </p>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row sm:items-center gap-3 bg-white p-3 rounded-2xl shadow-sm border border-neutral-300">
            <Tooltip text="Sets the probability level for the risk calculation. A 95% confidence means there's only a 5% chance daily losses will exceed the VaR estimate.">
              <span className="text-xs font-bold text-neutral-500 uppercase pl-1">Confidence Level</span>
            </Tooltip>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-neutral-300 overflow-hidden mb-2">
          <button
            onClick={() => setShowHelp(!showHelp)}
            className="w-full flex items-center justify-between p-4 bg-neutral-100 hover:bg-neutral-200 transition-colors"
          >
          ...
          {showHelp && (
            <div className="p-6 border-t border-neutral-300 grid grid-cols-1 md:grid-cols-3 gap-6 bg-white">
              <div>
                ...
                <p className="text-sm text-neutral-700 leading-relaxed">
                  ...
                  tool balances weights automatically. Watch KPI cards update in real-time.
                </p>
              </div>
              <div>
                <h4 className="font-bold text-neutral-900 text-sm mb-2 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div
            className={`relative bg-white border rounded-[2rem] p-6 shadow-sm transition-all duration-500 ${
              metrics.isHighRisk
                ? 'border-l-4 border-l-black border-neutral-500 bg-neutral-100'
                : 'border-neutral-300'
            }`}
          >
            {metrics.isHighRisk && (
            ...
            <div
              className={`text-3xl font-bold font-mono tracking-tight ${
                metrics.isHighRisk ? 'text-black' : 'text-neutral-900'
              }`}
            >
          ...
          <div className="bg-white border border-neutral-300 rounded-[2rem] p-6 shadow-sm">
            <div className="mb-2">
            ...
            <div className="text-3xl font-bold font-mono tracking-tight text-neutral-900">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2 px-2">
              ...
              <h3 className="text-sm font-bold text-neutral-800 uppercase tracking-wider">Asset Allocation</h3>
              <span
                className={`ml-auto text-xs font-bold px-3 py-1 rounded-full border ${
                  Math.abs(totalWeight - 100) < 0.2
                    ? 'text-neutral-900 bg-neutral-200 border-neutral-400'
                    : 'text-black bg-neutral-300 border-black'
                }`}
              >
            ...
            <div className="space-y-3 max-h-[450px] overflow-y-auto custom-scrollbar pr-2 pb-10">
              {assets.map((asset) => (
                <div
                  key={asset}
                  className="group relative bg-white p-5 rounded-[2rem] shadow-sm border border-neutral-300 hover:shadow-md transition-all"
                >
                  <button
                    onClick={() => handleRemoveAsset(asset)}
                    className="absolute right-4 top-4 p-2 bg-neutral-200 text-neutral-900 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-neutral-300"
                  >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Tooltip
                          ...
                          hideIcon={true}
                        >
                          <div className="flex items-center gap-1 bg-neutral-100 px-3 py-1 rounded-full text-[11px] font-bold text-neutral-800 hover:bg-neutral-200 transition-colors cursor-help border border-neutral-300">
                            <span>Vol (sigma):</span>
                            <input
                              type="number"
                              className="bg-transparent w-10 text-neutral-900 font-mono outline-none text-center cursor-text"
                    ...
                    <div className="flex flex-col items-end mr-8">
                      <div className="flex items-baseline gap-1">
                        <input
                          type="number"
                          className="bg-transparent text-right text-3xl font-bold font-mono text-neutral-900 w-20 outline-none focus:text-black transition-all"
                  ...
                  <div className="relative h-2 bg-neutral-200 rounded-full overflow-hidden mt-2">
                    <div
              ...
              <div className="flex gap-2 mt-4 items-center justify-center pt-2">
                <div className="flex flex-col sm:flex-row gap-2 bg-white p-3 rounded-2xl shadow-sm border border-neutral-300 w-full">
                  <input
                    type="text"
                    placeholder="Ticker (e.g. QQQ)"
                    value={newTicker}
                    onChange={(e) => setNewTicker(e.target.value.toUpperCase())}
                    className="bg-neutral-100 border border-neutral-300 rounded-xl px-4 py-2 text-sm font-bold text-neutral-900 uppercase outline-none focus:ring-2 focus:ring-neutral-400 w-full placeholder:text-neutral-500"
                  />
                  <div className="flex items-center bg-neutral-100 border border-neutral-300 rounded-xl px-4 py-2">
                    <span className="text-xs font-bold text-neutral-500 mr-2">VOL</span>
                    <input
                      type="number"
                      value={newVol}
                      onChange={(e) => setNewVol(parseFloat(e.target.value) || 0)}
                      className="bg-transparent w-14 outline-none text-sm font-bold font-mono text-neutral-900 text-center"
                    />
                    <span className="text-xs font-bold text-neutral-500 ml-1">%</span>
                  </div>
                  <button
                    onClick={handleAddAsset}
                    className="bg-black text-white hover:bg-neutral-800 shadow-md shadow-black/25 rounded-xl px-4 py-2 transition-all flex items-center justify-center gap-2"
                  >
          ...
          <div className="space-y-6">
            <div className="bg-white border border-neutral-300 rounded-[2rem] p-6 shadow-sm">
              ...
              <div className="bg-neutral-100 rounded-2xl p-4 border border-neutral-300 overflow-x-auto custom-scrollbar">
                <div className="min-w-max">
                  <div className="flex mb-2">
                    ...
                    {assets.map((col) => (
                      <div
                        key={`header-${col}`}
                        className="w-16 text-center text-xs font-bold text-neutral-500 uppercase"
                      >
                        {col}
                      </div>
                    ))}
                  ...
                  {assets.map((row, i) => (
                    <div key={`row-${row}`} className="flex mb-1.5 items-center">
                      <div className="w-16 text-xs font-bold text-neutral-700 text-right pr-4 uppercase">
                        {row}
                      </div>
                      {assets.map((col, j) => {
                        ...
                        if (!isDiagonal) {
                          if (val > 0.5) {
                            bgColor = 'bg-neutral-300 border-neutral-500';
                            textColor = 'text-black';
                          } else if (val > 0) {
                            bgColor = 'bg-neutral-200';
                            textColor = 'text-neutral-800';
                          } else if (val < -0.2) {
                            bgColor = 'bg-neutral-100 border-neutral-500';
                            textColor = 'text-black';
                          } else if (val < 0) {
                        ...
                        return (
                          <div key={`cell-${row}-${col}`} className="w-16 px-1">
                            {isDiagonal ? (
                              <div className="w-full text-center text-xs font-mono font-semibold py-2 text-neutral-500 bg-neutral-200 rounded-xl">
                                1.00
                              </div>
                            ) : (
                              <input
                                ...
                                className={`w-full text-center text-xs font-mono font-semibold py-2 rounded-xl outline-none transition-colors border border-neutral-300 ${bgColor} ${textColor} focus:ring-2 focus:ring-neutral-400 focus:border-black shadow-sm`}
                              />
                            )}
                          </div>
                        );
              ...
              <p className="text-[11px] text-neutral-700 mt-4 font-medium leading-relaxed bg-neutral-100 border border-neutral-300 p-3 rounded-xl">
                <span className="font-bold text-black">Note:</span> Negative correlations (e.g. SPY and
            ...
            <div className="bg-white border border-neutral-300 rounded-[2rem] p-6 shadow-sm">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="bg-neutral-100 rounded-2xl p-5 border border-neutral-300 flex items-center justify-between shadow-inner">
                  <div>
                  ...
                  <div className="text-4xl font-bold font-mono tracking-tight text-black">
                    {stressTestImpact > 0 ? '+' : ''}
