      ...
      <div className="max-w-6xl mx-auto space-y-6">
            ...
            <div className="p-6 border-t border-slate-100 grid grid-cols-1 md:grid-cols-3 gap-6 bg-white">
              <div>
                ...
                <p className="text-sm text-slate-600 leading-relaxed">
                  Adjust the allocation percentages to build your portfolio (e.g., mixing Stocks and Treasury Bonds). The
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <div className="space-y-1.5 max-h-[240px] overflow-y-auto custom-scrollbar pr-2 pb-2">
              {assets.map((asset) => (
                <div
                  key={asset}
                  className="group relative bg-white p-2 rounded-lg shadow-sm border border-slate-100 hover:shadow-md transition-all"
                >
                  <button
                    onClick={() => handleRemoveAsset(asset)}
                    className="absolute right-2 top-2 p-1 bg-rose-50 text-rose-500 rounded opacity-0 group-hover:opacity-100 transition-opacity hover:bg-rose-100"
                  >
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex flex-col gap-0.5 flex-1">
                      ...
                      <Tooltip
                        text="Annualized volatility (standard deviation) for this specific asset."
                        hideIcon={true}
                      >
                        <div className="flex items-center gap-0.5 bg-indigo-50 px-1.5 py-0.5 rounded text-[10px] font-bold text-indigo-700 hover:bg-indigo-100 transition-colors cursor-help border border-indigo-200 w-fit">
                          <span>Vol:</span>
                          <input
                            type="number"
                            className="bg-transparent w-6 text-indigo-700 font-mono outline-none text-center cursor-text text-[10px]"
                    ...
                    <div className="flex items-baseline gap-0.5">
                      <input
                        type="number"
                        className="bg-transparent text-right text-base font-bold font-mono text-slate-900 w-12 outline-none focus:text-indigo-600 transition-all"
              ...
              <div className="flex gap-1 mt-2 items-center justify-center pt-1">
                <div className="flex gap-1 bg-white p-1.5 rounded-lg shadow-sm border border-slate-100 w-full">
                  <input
                    type="text"
                    placeholder="TKR"
                    value={newTicker}
                    onChange={(e) => setNewTicker(e.target.value.toUpperCase())}
                    className="bg-slate-50 border border-slate-200 rounded px-2 py-1 text-xs font-bold text-slate-900 uppercase outline-none focus:ring-1 focus:ring-indigo-200 w-14 placeholder:text-slate-400"
                  />
                  <div className="flex items-center bg-slate-50 border border-slate-200 rounded px-2 py-1">
                    <span className="text-[10px] font-bold text-slate-400 mr-1">V</span>
                    <input
                      type="number"
                      value={newVol}
                      onChange={(e) => setNewVol(parseFloat(e.target.value) || 0)}
                      className="bg-transparent w-8 outline-none text-xs font-bold font-mono text-slate-900 text-center"
                    />
                    <span className="text-[10px] font-bold text-slate-400 ml-0.5">%</span>
                  </div>
                  <button
                    onClick={handleAddAsset}
                    className="bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm shadow-indigo-200 rounded px-2 py-1 transition-all flex items-center justify-center"
                  >
